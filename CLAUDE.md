...
Not covered: providers that stream reasoning in a *separate* field — `delta.reasoning_content` (DeepSeek, Qwen via vLLM), `delta.reasoning` (OpenRouter), and Anthropic `thinking_delta` blocks. `useSubmit.ts` reads only `delta.content` and `helper.ts` keeps only `text_delta`, so that reasoning is currently dropped. Wiring it up would mean an API-layer change plus a `MessageInterface` field, and *that* would need a migration.
