...
const ICON_TEMPLATE = 'icon-rounded-macos.png';
...
const createServer = () => {
  ...
  // Absolute path to the dist directory — used as the jail for all file serving
  ...
  const server = http.createServer((request, response) => {
    ...
    let filePath =
      request.url === '/'
        ...
        : path.resolve(path.join(distDir, request.url));

    // SECURITY: Reject any resolved path that escapes the dist directory
    if (
  ...
  // Listen on loopback only — prevents exposure to other devices on the local network
