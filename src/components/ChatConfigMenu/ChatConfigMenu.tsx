...
const ChatConfigPopup = ({
  ...
  return (
    ...
    >
      <div className='p-6 border-b border-[var(--border-mid)] w-full text-sm text-[var(--fg)]'>
        <DefaultSystemChat
          ...
          <div className='flex items-center justify-between'>
            ...
            <button
              ...
              className={`relative ml-4 shrink-0 inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--accent)] ${
                _webSearch ? 'bg-[var(--accent)]' : 'bg-[var(--bg-sand)]'
              }`}
            >
              <span
        ...
        <button
          type='button'
          className='mt-6 text-xs px-3 py-1.5 rounded-lg bg-[var(--bg-sand)] text-[var(--fg-2)] hover:bg-[var(--ring)] transition-colors cursor-pointer'
          onClick={handleReset}
        >
          {t('resetToDefault')}
        </button>
  ...
  return (
    <div>
      <div className='flex items-center justify-between mb-1'>
        <div className='block text-sm font-medium text-[var(--fg)]'>
          {t('defaultSystemMessage')}
        </div>
        <PromptLibraryPicker onSelect={_setSystemMessage} />
      </div>
      <textarea
        className='my-2 mx-0 px-3 resize-none rounded-lg bg-[var(--bg-card)] text-[var(--fg)] overflow-y-hidden leading-7 p-1.5 border border-[var(--border-mid)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] w-full max-h-10 transition-all placeholder-[var(--fg-3)]'
