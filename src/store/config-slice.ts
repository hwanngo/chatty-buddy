...
export interface ConfigSlice {
  ...
  // When true, the default chat model auto-tracks the latest OpenAI flagship as
  // the model list updates. An explicit default-model change turns this off.
  autoModel: boolean;
...
export const createConfigSlice: StoreSlice<ConfigSlice> = (set, get) => ({
  ...
  setDefaultChatConfig: (defaultChatConfig: ConfigInterface) => {
    set((prev: ConfigSlice) => ({
      ...
      // An explicit change to the default model opts out of auto-tracking;
      // changing only other fields (temperature, etc.) leaves it untouched.
      autoModel:
        defaultChatConfig.model === prev.defaultChatConfig.model
          ? prev.autoModel
          : false,
    }));
  },
