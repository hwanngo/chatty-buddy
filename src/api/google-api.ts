...
const getDriveFileTyped = async (
  fileId: string,
  accessToken: string
): Promise<StorageValue<any>> => {
...
export const validateGoogleOath2AccessToken = async (accessToken: string) => {
  // Send the token in the Authorization header (never in the URL — query
  // strings leak into history, proxy logs, and Referer headers).
  const response = await fetch(
