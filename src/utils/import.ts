...
export const convertOpenAIToChattyBuddyFormatPartialOK = (
  openAIChatExport: any
): ChatInterface => {
  return convertOpenAIToChattyBuddyFormat(openAIChatExport, true);
};

export const convertOpenAIToChattyBuddyFormatPartialNTY = (
  openAIChatExport: any
): ChatInterface => {
  return convertOpenAIToChattyBuddyFormat(openAIChatExport, false);
};
// Convert OpenAI chat format to chatty-buddy format
export const convertOpenAIToChattyBuddyFormat = (
  openAIChatExport: any,
  ...
  if (isOpenAIChat(openAIChatExport)) {
    ...
    while (deepestNode) {
      ...
      console.log(
        `Backtracking node with id ${deepestNode.id} at depth ${currentDepth}`
      );
      ...
      if (deepestNode.message) {
        ...
        if (Array.isArray(content.parts)) {
          ...
          if (textContent.length > 0) {
            ...
            console.log(
              `Node with id ${deepestNode.id} has empty text content.`
            );
      ...
      console.log(
        `Moving from node ${deepestNode.id} to parent node ${parentNodeId}`
      );
      deepestNode = parentNodeId
        ? openAIChatExport.mapping[parentNodeId]
        : null;
      currentDepth++;
    }
    ...
    // Show differences
    const diffDeepestToMessages = deepestPathIds.filter(
      (id) => !messageIds.includes(id)
    );
    console.log(
      'Difference between Deepest Path IDs and Message IDs:',
      diffDeepestToMessages
    );

    // Check if the difference between diffDeepestToMessages and emptyOrNullMessageIds is empty
    const diffDeepestToMessagesAndEmpty = diffDeepestToMessages.filter(
      (id) => !emptyOrNullMessageIds.includes(id)
    );
    console.log(
      'Difference between diffDeepestToMessages and Empty or Null Message IDs:',
      diffDeepestToMessagesAndEmpty
    );
...
// Import OpenAI chat data and convert it to chatty-buddy format
export const importOpenAIChatExport = (
  ...
  if (Array.isArray(openAIChatExport)) {
    if (shouldAllowPartialImport) {
      return openAIChatExport.map(convertOpenAIToChattyBuddyFormatPartialOK);
    } else {
      return openAIChatExport.map(convertOpenAIToChattyBuddyFormatPartialNTY);
    }
